...
This specification defines the behavior of the Outscale IAM Terraform module, which manages Outscale EIM (External Identity Management) resources including users, groups, policies, and access keys.
...
- THEN outputs include `user_id`, `user_name`, `creation_date`, `path`, and `user_email`
...
- WHEN the module completes
- THEN outputs include `user_group_id`, `user_group_name`, `orn`, `path`, and `creation_date`

#### Scenario: Policy outputs

- GIVEN policies are created
- WHEN the module completes
- THEN outputs include `policy_id`, `policy_name`, `orn`, `policy_default_version_id`, and `creation_date`
